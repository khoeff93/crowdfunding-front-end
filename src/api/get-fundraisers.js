async function getFundraisers() {
    // First we create the URL for the request by using the Vite (takes our react app and makes it Javascript) environment variable and the API endpoint.
    ...
    if (!response.ok) {
      ...
      // data?. says see if it has details in, and ?? is id it doesn't then fall back error.
